...
class Decode_Image(nn.Module):
    def __init__(
        ...
        self.a = nn.Sequential(
            nn.Linear(
                ...
                out_features = hidden_state_size),
            nn.PReLU(),
            nn.Linear(
                in_features = hidden_state_size,
                out_features = 16 * args.image_size * args.image_size),
            nn.PReLU())
        
        example = self.a(example)
        if(verbose): 
            print("\ta:", example.shape)
        example = example.reshape(example.shape[0], 16, args.image_size, args.image_size)
        if(verbose): 
            print("\tReshaped:", example.shape)
        ...
        example_output = example_output.reshape(episodes, steps, args.image_size, args.image_size, 4)
    ...
    def forward(self, hidden_state):
        ...
        a = a.reshape(episodes * steps, 16, args.image_size, args.image_size)
        ...
        output = output.reshape(episodes, steps, args.image_size, args.image_size, 4)
